...
class Bybit_api:
    BASE_LINK = 'https://api-demo.bybit.com'
    ...
    def http_request(self, method, endpoint, params):
        ...
        if method == 'GET':
            ...
            response = requests.get(url=self.BASE_LINK + endpoint, data=json.dumps(params), headers=self.header)
        elif method == 'POST':
    ...
    def post_market_order(self, symbol: str, side: str, qnt, reduce_only=False, headers=False):
        ...
        response_json, respponse_headers = self.http_request(method=method, endpoint=endpoint, params=params)
        if headers:
            return response_json, respponse_headers
        else:
            return response_json
